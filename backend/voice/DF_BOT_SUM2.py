...
class Bot:
    ...
    def recognize_speech(self, speech_text):
        # 음성 인식 결과를 저장
        self.response = self.speech_client.recognize(
            ...
            audio=speech.RecognitionAudio(content=speech_text)
        )
    ...
    def save_parameters_to_db(self, response):
        ...
        try:
            # 테이블 삭제 후 다시 생성
            self.cur = self.conn.cursor()  # 커서를 다시 열기
            ...
            for key in collected_params.keys():
                if key not in existing_columns:
                    alter_table_query = f"ALTER TABLE hamburger ADD COLUMN {key} CHARACTER(255)"
